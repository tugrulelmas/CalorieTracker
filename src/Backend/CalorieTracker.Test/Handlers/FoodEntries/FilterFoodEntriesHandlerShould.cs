...
namespace CalorieTracker.Test.Handlers.FoodEntries {
    public class FilterFoodEntriesHandlerShould {
        ...
        public async Task ReturnEmptyWhenTheUserDoesntExist() {
            ...
            using (var context = new CalorieTrackerContext(options)) {
                var filterFoodEntriesHandler = new FilterFoodEntriesHandler(context);
                var filterFoodEntriesRequest = new FilterFoodEntriesRequest(1, 10, Guid.NewGuid(), DateTime.Today, DateTime.Today);

                var result = await filterFoodEntriesHandler.Handle(filterFoodEntriesRequest, CancellationToken.None);

                Assert.NotNull(result);
                Assert.Equal(0, result.TotalCount);
                Assert.Empty(result.FoodEntries);
            }
        }
        ...
        public async Task ReturnEmptyWhenTheUserDoesntHaveAnyFoodEntry() {
            ...
            using (var context = new CalorieTrackerContext(options)) {
                var filterFoodEntriesHandler = new FilterFoodEntriesHandler(context);
                var filterFoodEntriesRequest = new FilterFoodEntriesRequest(1, 10, userId, DateTime.Today, DateTime.Today);
        ...
        public async Task ReturnFoodEntries() {
            ...
            using (var context = new CalorieTrackerContext(options)) {
                var filterFoodEntriesHandler = new FilterFoodEntriesHandler(context);
                var filterFoodEntriesRequest = new FilterFoodEntriesRequest(1, 10, userId, DateTime.Today.AddDays(-4), DateTime.Today);

                var result = await filterFoodEntriesHandler.Handle(filterFoodEntriesRequest, CancellationToken.None);

                Assert.NotNull(result);
                Assert.Equal(4, result.TotalCount);
                Assert.NotEmpty(result.FoodEntries);
                Assert.Equal(4, result.FoodEntries.Count());
            }
        }
        ...
        public async Task ReturnFoodEntriesByThePageSize() {
            ...
            using (var context = new CalorieTrackerContext(options)) {
                var filterFoodEntriesHandler = new FilterFoodEntriesHandler(context);
                var filterFoodEntriesRequest = new FilterFoodEntriesRequest(1, 10, userId, DateTime.Today.AddDays(-20), DateTime.Today);
                ...
                Assert.NotEmpty(result.FoodEntries);
                Assert.Equal(filterFoodEntriesRequest.PageSize, result.FoodEntries.Count());
            }
        }
    }
}
