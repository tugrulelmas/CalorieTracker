...
export default function HomePage() {
  ...
  const list = async (page, pageSize, from, to) => {
    setLoading(true);
    const data = await foodEntryService.getMyFoodEntries(page, pageSize, from, to)
      .catch(e => {
  ...
  return (
    <div className="container px-6 mx-auto grid">
      ...
      <AddFoodEntry isOpen={isModalOpen} close={closeModal} />

      <DateFilter onList={listFoodEntries} loading={loading} />

      <FoodEntryList foodEntries={foodEntriesResult.foodEntries} pageSize={filter.pageSize} page={filter.page} totalCount={foodEntriesResult.totalCount} onPageChange={onPageChange} />
